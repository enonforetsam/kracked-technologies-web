...
export default function KrackedHostingPage({ graph }) {
  const node = graph.nodes.find(n => n.id === 'kracked-hosting')
  const html = useMemo(() => node ? renderContent(node.content, graph.nodes) : '', [node, graph.nodes])

  return (
    <div className="competitors-page">
      <PageHeader
        eyebrow="INFRASTRUCTURE · PROPOSAL · APRIL 2026"
        title="Kracked Hosting"
        subtitle="A community-owned deployment network for Malaysian builders. Vercel-like workflow, Malaysian-run infrastructure, on-chain coordination when the volume earns it. Kracked Chain starts here."
        visual="hosting"
      />
      ...
      {node ? (
        ...
        <div className="mc-card glass mc-card-wide" style={{ padding: 40, textAlign: 'center' }}>
          <p style={{ color: 'var(--text-tertiary)' }}>Kracked Hosting whitepaper not found in vault.</p>
        </div>
      )}
    </div>
  )
